 ...
 */
var promptBaseUrl = "../Resources/Prompts/en-IN/";
 ...
 */
function enINPlayBuiltinType(value, type, promptUrl, format)
{
...
//Custom code    
    ...
    if(bool4==1)
    {
     	promptsArray = NumberPrompts(fst4).concat(magnitudePrompts(5), promptsArray);
    }
    if(bool3==1)
    {
       	promptsArray = NumberPrompts(fst3).concat(magnitudePrompts(5), promptsArray);
    }            	       
    if(bool2==1)
    {
       	promptsArray = NumberPrompts(fst2).concat(magnitudePrompts(6), promptsArray);
    }
    if(bool1==1)
    {
       	promptsArray = NumberPrompts(fst1).concat(magnitudePrompts(6), promptsArray);
    }            	       
...
//custom code
function NumberPrompts(fst)
{
	var promptsArray = new Array();
	if(fst>=1 && fst<=9)
	{
 		promptsArray.push(promptBaseUrl + "cardinals/" + "00" + fst + audioFileExtension);
	}
	else
	{
		promptsArray.push(promptBaseUrl + "cardinals/" + "0" + fst + audioFileExtension);
	}
return promptsArray;
}
...
function hiINPlayBuiltinType(value, type, promptUrl, format)
{
    type = type.toLowerCase();
    // Sets the new base URL that is prepended to the URLs for all audio files.
    if (!(typeof promptUrl == 'undefined'))
